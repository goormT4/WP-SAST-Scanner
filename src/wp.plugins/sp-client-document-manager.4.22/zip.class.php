...
class Zip {
    ...
    function __construct($archiveName = "", $contentType = "application/zip") {
        if (!function_exists('sys_get_temp_dir')) {
            die ("ERROR: Zip " . self::VERSION . " requires PHP version 5.2.1 or above.");
        }
     ...
     */
		...
		if (!is_null($this->streamFilePath)) {

			rewind($this->streamFilePath);

			while(!feof($this->streamFilePath)) {

				fwrite($fd, fread($this->streamFilePath));

			}

				

			fclose($this->streamFilePath);

		} else {

			fwrite($fd, $this->streamData);
     ...
     * @return bool $success
     */
    public function finalize($write=false) {
        if (!$this->isFinalized) {
            if (strlen($this->streamFilePath) > 0) {
                $this->closeStream();
            }
